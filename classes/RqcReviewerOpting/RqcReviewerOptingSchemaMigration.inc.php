...
class RqcReviewerOptingSchemaMigration extends Migration
{
    ...
    {
		Capsule::schema()->create('rqc_reviewer_opting', function (Blueprint $table) {
		...
		Capsule::schema()->create('rqc_reviewer_opting_settings', function (Blueprint $table) {
            $table->bigIncrements('rqc_reviewer_opting_id');
    ...
    {
		Capsule::schema()->drop('rqc_reviewer_opting_settings');
		RqcLogger::logInfo("Dropped table 'rqc_reviewer_opting_settings' in the database");
		Capsule::schema()->drop('rqc_reviewer_opting');
